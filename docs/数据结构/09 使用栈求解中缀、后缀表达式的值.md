...
![img](../assets/images/9798817c7c0b4b5f3e9a96886e9957b5.png)
...
![img](../assets/images/063059b3d8e87a4ae9297f585221996e.png)
...
![img](../assets/images/5a6ef3099d90e8825f2e403df7b7b708.png)
...
![img](../assets/images/8a418c5fddd37980e76122891e04a07e.png)

- 因 `-(减法)`运算符优先级低于`optStack`栈顶的`*`运算符。这时从`optStack`栈中弹出`*`，再从`numStack`中弹出`3`和`3` 两个操作数，进行乘法运算`3*3=9`，并把结果压入`numStack`栈中。

![img](../assets/images/b7b2bcb97fb8ac523e8b4bdc99ca8e8a.png)

- 计算完成后，因`-（减法）`和`+（加法）`的优先级相同，栈内优先。此时，把`+`从`optStack`栈中弹出，并从`numStack`中相继弹出`9`和`3`，计算`3+9=12`，并把结果压入`numStack`栈中。

![img](../assets/images/ce4015638d9b4ddcfa773fcb36f38990.png)

- 因`-(减法)`优先级大于栈中`(`的优先级，`-`入栈。

![img](../assets/images/df9defb28c9b21a60850a4de39d83866.png)

- 继续扫描，直到遇到右括号。

![img](../assets/images/08e36dc2b39f39e11af222f291fbdb22.png)
...
![img](../assets/images/9b09aa1657a8d545912c767e874aca77.png)

弹出`-`运算符，并对`numStack`栈中的`12`和`6`进行计算。

![img](../assets/images/4a511b84e0a258a0e2e8c8df0d70459d.png)

- `(`出栈，表示由括号表示的子表达式计算结束。继续扫描到第二个`-`

![img](../assets/images/3e2dd0ce3557c5a057dca9a12d5700ed.png)

- 因`-`优先级小于`^`，先做`6^6=46656`乘方运算 。

![img](../assets/images/48dc97690ae41183cca95f89306354e5.png)

- `-`优先级小于`*`，继续做乘法运算，`46656*4=186624`。

![img](../assets/images/eb70abf599c7c00c4a5451704d2202a5.png)

- `-`入栈，最后一个数字 `8`入栈。

![img](../assets/images/922437b9226110c69495e40707dc098d.png)

- 因整个表达式结束，弹出`-`，做最后的减法运算`186624-8=186616`。整个表达式结束，`numStack`栈顶的结果为表达式的最后结果。

![img](../assets/images/42b0309402152ca5750be6eeac73346c.png)
...
![img](../assets/images/c160e533f9639c8cabe851e587173567.png)

- 扫描中缀表达式，字符`8`直接输出，`+`是第一个操作数，因可能后续有更高的运算符，入栈。

![img](../assets/images/74d528bc0ab02c8678472fea2bad23cc.png)

- 字符`5`直接输出，`*`优先级大于栈顶`+`优先级，入栈。

![img](../assets/images/a6880ff110a451f31d5faa4defcd6825.png)

- `(`运算符在栈外优先级最高，入栈。

![img](../assets/images/b12419a7217cabfa833f8329ec8fda9a.png)

- 字符`7`直接输出，因`(`运算符在栈内优先级最低，`-`运算符入栈。

![img](../assets/images/924d495e11c2f18b1a1963eecd3bacfa.png)

- 字符`1`直接输出，`)`栈外优先级最低。运算符出栈，一直碰到`(`。

![img](../assets/images/ebfd9f4a5efaee19d6f75369e0463815.png)

- `-`运算符小于栈中的`+`、`+`运算符。`*`、`+`运算符出栈。`-`入栈。

![img](../assets/images/f8512b85771777814bd56283a870a88e.png)

- `/`优先级大于`-`，入栈。字符直接输出。

![img](../assets/images/2bdeaf94435585151459799017a2416c.png)

- 字符扫描结束，把运算符栈中的运算符全部出栈。

![img](../assets/images/2e15e714141ff69d6b3fd85b8ebf6520.png)
...
![img](../assets/images/02a741e95fa5751dd9111987a1c3a659.png)
